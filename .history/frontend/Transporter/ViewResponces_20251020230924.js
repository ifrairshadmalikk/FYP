...
export default function ViewResponse({ navigation, route }) {
    ...
    // Initialize data from route params or use defaults
    useEffect(() => {
        if (pollData) {
            setPoll(pollData);
            if (initialResponses) {
                setPassengerResponses(initialResponses);
            } else {
                // Generate mock responses based on poll data
                generateMockResponses(pollData);
            }
        } else {
            // Default data if no pollData provided
            setPoll({
                id: "1",
                date: new Date(),
    ...
    const generateMockResponses = (pollDataToUse = poll) => {
        ...
        const selectedNetworks = pollDataToUse.networks.filter(net => net.selected);
        const timeSlots = pollDataToUse.timeSlots;
        
        // Generate responses proportional to network size
        selectedNetworks.forEach(network => {
            const responseCount = Math.floor((network.passengers / pollDataToUse.totalPassengers) * 20) + 5;
            
            for (let i = 0; i < responseCount; i++) {
                const randomTimeSlot = timeSlots[Math.floor(Math.random() * timeSlots.length)];
                const status = Math.random() > 0.3 ? 'confirmed' : (Math.random() > 0.5 ? 'pending' : 'cancelled');
                ...
                responses.push({
                    ...
                    responseTime: new Date(Date.now() - Math.floor(Math.random() * 86400000)),
                    status: status
                });
            }
        });
        ...
        if (isNewPoll && poll) {
            const interval = setInterval(() => {
                setPassengerResponses(prev => {
                    if (prev.length >= poll.totalPassengers * 0.8) {
            ...
            }, 5000); // New response every 5 seconds
    ...
    const handleSendReminder = () => {
        const pendingCount = poll ? poll.totalPassengers - responseStats.total : 0;
        Alert.alert(
            "Send Reminder",
            ...
            <View style={styles.headerContent}>
                ...
                <View style={styles.headerStats}>
                    <Text style={styles.headerStat}>
                        📅 {poll ? new Date(poll.date).toLocaleDateString() : 'Loading...'}
                    </Text>
                    <Text style={styles.headerStat}>
                ...
                <View style={styles.statsGrid}>
                    ...
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statSubtext}>
                            {responseStats.total > 0 ? Math.round((responseStats.confirmed / responseStats.total) * 100) : 0}%
                        </Text>
                    </View>
                    <View style={styles.statCard}>
                    ...
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statSubtext}>
                            {responseStats.total > 0 ? Math.round((responseStats.cancelled / responseStats.total) * 100) : 0}%
                        </Text>
                    </View>
                </View>
