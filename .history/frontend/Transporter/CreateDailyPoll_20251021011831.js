...
export default function CreateDailyPoll({ navigation }) {
    ...
    // Generate mock passenger responses based on poll data
    const generateMockResponses = (pollData) => {
        const responses = [];
        const names = ["Ali Ahmed", "Sara Khan", "Usman Malik", "Fatima Noor", "Bilal Raza", "Ayesha Siddiqui", "Omar Farooq", "Zainab Ali"];
        const statuses = ["confirmed", "pending", "cancelled"];
        
        pollData.networks.forEach(network => {
            if (network.selected) {
                // Generate 3-8 responses per network
                const responseCount = Math.floor(Math.random() * 6) + 3;
                
                for (let i = 0; i < responseCount; i++) {
                    const randomTimeSlot = pollData.timeSlots[Math.floor(Math.random() * pollData.timeSlots.length)];
                    ...
                    responses.push({
                        ...
                        responseTime: new Date(Date.now() - Math.floor(Math.random() * 86400000)), // Random time in last 24 hours
                        status: randomStatus
                    });
                }
            }
        ...
        Alert.alert(
            ...
            [
                ...
                { 
                    text: "Send Poll", 
                    onPress: () => {
    // Convert date (if exists) to string before navigating
    const safePollData = {
        ...pollData,
        date: pollData.date ? pollData.date.toISOString() : null,
    };

    navigation.navigate('ViewResponse', { 
        pollData: safePollData,
