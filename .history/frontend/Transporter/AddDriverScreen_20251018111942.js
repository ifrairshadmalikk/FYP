...
export default function AddDriverSearch({ navigation }) {
  ...
  const locations = ["Lahore", "Karachi", "Islamabad", "Faisalabad", "Multan", "Peshawar"];
  const vehicleTypes = ["Sedan", "Hatchback", "SUV", "Hybrid"];
  ...
  // Filter drivers based on search, active tab and filters
  const filteredDrivers = drivers.filter((d) => {
    ...
    // Additional filters
    const matchesLocation = !filters.location || d.location === filters.location;
    const matchesVehicleType = !filters.vehicleType || d.vehicleType === filters.vehicleType;
    const matchesExperience = !filters.experience || d.experience === filters.experience;
    const matchesRating = !filters.rating || (
      filters.rating === "4.0+" && parseFloat(d.rating) >= 4.0 ||
      filters.rating === "4.5+" && parseFloat(d.rating) >= 4.5 ||
      filters.rating === "4.8+" && parseFloat(d.rating) >= 4.8
    );

    return matchesSearch && matchesTab && matchesLocation && matchesVehicleType && matchesExperience && matchesRating;
  });
            ...
            <View style={styles.filterOptions}>
              {vehicleTypes.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.filterOption,
                    filters.vehicleType === type && styles.selectedFilterOption
                  ]}
                  onPress={() => setFilters(prev => ({ ...prev, vehicleType: type }))}
                >
                  <Text style={[
                    styles.filterOptionText,
                    filters.vehicleType === type && styles.selectedFilterOptionText
                  ]}>
                    {type}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles.filterOptions}>
              {experienceOptions.map((exp) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.filterOption,
                    filters.experience === exp && styles.selectedFilterOption
                  ]}
                  onPress={() => setFilters(prev => ({ ...prev, experience: exp }))}
                >
                  <Text style={[
                    styles.filterOptionText,
                    filters.experience === exp && styles.selectedFilterOptionText
                  ]}>
                    {exp}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles.filterOptions}>
              {ratingOptions.map((rating) => (
                ...
                >
                  <Text style={[
                    styles.filterOptionText,
                    filters.rating === rating && styles.selectedFilterOptionText
                  ]}>
                    {rating}
                  </Text>
                </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        >
          <Ionicons name="filter" size={20} color={isFilterActive ? "#afd826" : "#fff"} />
          ...
          {Object.entries(filters).map(([key, value]) => 
            value && (
              <View key={key} style={styles.activeFilterTag}>
                ...
                <TouchableOpacity onPress={() => setFilters(prev => ({ ...prev, [key]: "" }))}>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {search || isFilterActive ? "Try adjusting your search or filter terms" : "No driver requests available"}
            </Text>
          </View>
        }
      />
